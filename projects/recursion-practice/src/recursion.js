...
//if n is less than zero
  if (n < 0) {
    ...
    //return 1
    return 1; 
    //otherwise return n multiplied by the factorial function of n - 1
  } else {
...
var exponent = function(base, exp, result = 1) {
  if (exp === 0) {
    ...
    result = base * exponent(base, exp - 1, result);
    return result;
  } else if (exp < 0) {
    return 1 / exponent(base, -exp, result);
  }
};
...
var reverse = function(string, result =[]) {
  if(string.length === 0){
    ...
    return reverse(string.slice(0, string.length - 1), result)
  }
};

// 10. Write a function that determines if a string is a palindrome.
var palindrome = function(string) {
  var store = string.replace(/\s/g, '')
  if (store.length <= 1) {
    return true;
  }
  if (store[0].toLowerCase() !== store[store.length - 1].toLowerCase()) {
    return false;
  }
  return palindrome(store.slice(1, -1));
};
...
// JavaScript's Math object.
var divide = function(x, y, n = 0) {
if(x < y){
  return n;
} else return divide(x - y, y, n++)
...
var countKeysInObj = function(obj, key, count=0) {
  ...
  if(arr.length === 0){
    ...
    else return countKeysInObj(obj, key, count, arr);
  }
};
...
var capitalizeFirst = function(input, output=[]) {
  if(input.length === 0){
    ...
    output.push(input[0][0].toUpperCase() + input[0].slice(1))
...
var minimizeZeroes = function(array, result=[]) {
  if(array.length === 0){
    return result;
  }else if(array[0] === 0 && array[1] === array[0]){
  return compress(array.slice(1), result)
  }
  else{
    result.push(array[0]);
...
// if the array is empty, return the result
  if (array.length === 0) {
    return result;
  }
  //if the first item in the array is negative and the count is even push the item with the sign switched
  if (array[0] < 0 && count % 2 === 0) {
    result.push(array[0] * -1);
//if the first item in the array is positive and the count is odd, push the item with the sign switched
  } else if (array[0] > 0 && count % 2 === 1) {
    result.push(array[0] * -1);
//otherwise push the item
  ...
  //increase count
  count++
  //return the function called on the array with the first item removed, the result array, and the count
